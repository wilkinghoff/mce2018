...
def fit_plda_model(data, labels_cat, numiter, vdim, udim, rcond=1e-6):
    """
...
def get_plda_scores(model, data1, data2):
    """
...
if __name__ == '__main__':
    ...
    plda_model = fit_plda_model(train_data, train_labels_cat, numiter=numiter, vdim=vdim, udim=udim)

    # compute scores for test data
    print('Computing test scores')
    scores = get_plda_scores(plda_model, enrol_data, test_data)

    # check if predictions are correct
    pred = np.argmax(scores, axis=0)
    pred_labels = np.unique(enrol_labels)[np.floor(pred/5).astype(np.int32)] # as there are 5 samples per class
    print('Accuracy: '+str(np.sum(pred_labels==test_labels.transpose())/len(pred_labels)*100)+' percent')
